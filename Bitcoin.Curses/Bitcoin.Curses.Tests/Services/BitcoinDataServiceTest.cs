...
namespace Bitcoin.Curses.Tests.Services
{
    ...
    public class BitcoinDataServiceTest
    {
        ...
        public void GetExchangeRatesAsync_NoDataExcpectedWhenJsonAreEmptyNegativeTest()
        {
            ...
            var cashService = Substitute.For<ICacheService>();
            networkService.IsInternetAvailable().Returns(Task.FromResult<bool>(true));

            var bitcoinService = new BitcoinDataService(dataService, rateSettingsApplyService, customCurrencySymbolService, networkService,cashService);
            var result = bitcoinService.GetExchangeRatesAsync(true).Result;
